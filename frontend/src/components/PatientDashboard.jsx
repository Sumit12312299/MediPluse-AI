    ...
    <div className="space-y-6 animate-slide-up max-w-7xl mx-auto">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-white border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="space-y-2">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-sky-50 text-sky-700 text-xs font-bold border border-sky-100">
          ...
          <p className="text-sm text-slate-600 font-medium">
            Next scheduled consultation: <strong className="text-slate-900 font-bold">Tomorrow at 10:30 AM</strong> with Dr. Rajesh Sharma (Cardiology).
          </p>
        </div>

        <button
          onClick={onOpenBooking}
          className="px-5 py-3 rounded-2xl bg-sky-600 hover:bg-sky-700 text-white font-bold text-sm transition-all btn-minimal shadow-md flex items-center space-x-2 self-start md:self-auto"
        >
      ...
      {/* Bento Grid KPI Metrics (4 Clean Cards) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* Bento Card 1: Live OPD Queue Token */}
        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs hover:border-sky-300 transition-all space-y-3">
          ...
          <div>
            ...
            <span className="text-xs text-slate-500 font-medium">Current Serving: # 11</span>
          </div>
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs hover:border-teal-300 transition-all space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
          ...
          <div>
            <p className="text-3xl font-extrabold text-slate-900">2 <span className="text-xs font-semibold text-slate-400">PDFs</span></p>
            <span className="text-xs text-slate-500 font-medium">Blood Test & ECG</span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs hover:border-indigo-300 transition-all space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
          ...
          <div>
            ...
            <span className="text-xs text-slate-500 font-medium">Confirmed Slot</span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200/80 shadow-xs hover:border-purple-300 transition-all space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
          ...
          <div>
            ...
            <span className="text-xs text-slate-500 font-medium">Voice Synthesized</span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-1.5 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {appointments && appointments.length > 0 ? (
            appointments.map(app => (
              <div key={app.id} className="p-6 rounded-3xl bg-white border border-slate-200/80 shadow-xs hover:shadow-md transition-all space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {prescriptions && prescriptions.length > 0 ? (
            prescriptions.map(rx => (
              <div key={rx.id} className="p-6 rounded-3xl bg-white border border-slate-200/80 shadow-xs hover:shadow-md transition-all space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-3.5">
